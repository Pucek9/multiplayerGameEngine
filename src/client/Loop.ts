...
function Loop(socket, user, screen, cursor: Cursor, map) {
    ...
    socket.on(API.GET_PLAYERS_STATE, function (_players: PlayerModel[]) {
        players
            .forEach(player => {
                ...
                if (currentPlayer && _player && _player.id === currentPlayer.id) {
                    const diff = {
    ...
    cursor.init(screen);
    // if (currentPlayer) {
    //     cursor.x = currentPlayer.x;
    //     cursor.y = currentPlayer.y;
    // }
    ...
    this.run = function () {
        ...
        // if (config.menu && currentPlayer && map) {
        //     [map, ...bullets, ...staticObjects, ...players].forEach(object => object.render(currentPlayer));
        // } else {
        //     menu.render();
        // }
